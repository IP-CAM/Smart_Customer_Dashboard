...
class ControllerExtensionModuleGixOCAccount extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('module_gixocaccount', $this->request->post);
		...
		if (isset($this->request->post['module_gixocaccount_status'])) {
			$data['module_gixocaccount_status'] = $this->request->post['module_gixocaccount_status'];
		} else {
			$data['module_gixocaccount_status'] = $this->config->get('module_gixocaccount_status');
		}

		if (isset($this->request->post['module_gixocaccount_css'])) {
			$data['module_gixocaccount_css'] = $this->request->post['module_gixocaccount_css'];
		} else {
			$data['module_gixocaccount_css'] = $this->config->get('module_gixocaccount_css');
		}

		if (isset($this->request->post['module_gixocaccount_back'])) {
			$data['module_gixocaccount_back'] = $this->request->post['module_gixocaccount_back'];
		} else {
			$data['module_gixocaccount_back'] = $this->config->get('module_gixocaccount_back');
		}
		...
		// Groups customer
		if (isset($this->request->post['module_gixocaccount_groups_customer'])) {
			$data['module_gixocaccount_groups_customer'] = $this->request->post['module_gixocaccount_groups_customer'];
		} else {
			$data['module_gixocaccount_groups_customer'] = $this->config->get('module_gixocaccount_groups_customer');
		}
		// End groups customer
		...
		if (isset($this->request->post['module_gixocaccount_el_customer'])) {
			$results = $this->request->post['module_gixocaccount_el_customer'];
			...
			foreach ($results as $result) {
				...
				$data['module_gixocaccount_el_customer'][] = array(
		...
		} elseif ($this->config->get('module_gixocaccount_el_customer')) {
			$results = $this->config->get('module_gixocaccount_el_customer');
		...
		if (isset($this->request->post['module_gixocaccount_module_customer'])) {
			$data['module_gixocaccount_module_customer'] = $this->request->post['module_gixocaccount_module_customer'];
		} elseif ($this->config->get('module_gixocaccount_module_customer')) {
			$data['module_gixocaccount_module_customer'] = $this->config->get('module_gixocaccount_module_customer');
		} else {
			$data['module_gixocaccount_module_customer'] = array();
		}
	...
	public function install(){
		...
		$setting = array(
			'module_gixocaccount_status'           => 0,
			'module_gixocaccount_css'              => '',
			'module_gixocaccount_back'             => 0,
			'module_gixocaccount_groups_customer'  => $gixocaccount_groups_customer,
			'module_gixocaccount_el_customer'      => $gixocaccount_el_customer,
			'module_gixocaccount_module_customer'  => $gixocaccount_module_customer
		);
		$this->model_setting_setting->editSetting('module_gixocaccount', $setting, 0);
	...
	public function uninstall(){
		$this->load->model('setting/setting');
		$this->model_setting_setting->deleteSetting('module_gixocaccount');
	}
