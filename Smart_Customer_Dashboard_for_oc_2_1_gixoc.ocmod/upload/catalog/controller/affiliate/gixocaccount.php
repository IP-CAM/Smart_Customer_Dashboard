...
class ControllerAffiliateGixOCAccount extends Controller {
	public function index() {
		...
		if (!$status) {
			return $this->load->controller('affiliate/account');
		}
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/gixocaccount.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/module/gixocaccount.tpl', $data));
		} else {
			$this->response->setOutput($this->load->view('default/template/module/gixocaccount.tpl', $data));
		}
	}
}